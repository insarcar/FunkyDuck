...
h(2) = nexttile(t, 2);
plot(Z,X), title 'signal(CoBE peak)';
hold on
plot(Z,Y1);
plot([Z(1), Z(end)], [plot_thresh, plot_thresh], '--r', 'LineWidth', 2);
...
h(3) = nexttile(t, 3);
plot(Z, GRed, "Color", 'b'); title 'post filter GR, blue = crest, red = CoBE peak';
hold on
...
h(4) = nexttile(t, 4);
plot(Z, factor); title 'crest factor';
grid on

h(5) = nexttile(t, 5);
plot(Z, factor1); title 'CoBE peak';
grid on

h(6) = nexttile(t, 6);
plot(Z,atks,"Color", 'b'); title 'time constants (ms) (crest)';
hold on
plot(Z,rels,"Color", 'r');
hold off
grid on
...
h(7) = nexttile(t, 7);
plot(Z,atks1,"Color", 'b'); title 'time constants (ms) (CoBE peak)';
hold on
